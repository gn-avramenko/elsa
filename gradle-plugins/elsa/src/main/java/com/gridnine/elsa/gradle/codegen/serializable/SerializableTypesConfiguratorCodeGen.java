...
package com.gridnine.elsa.gradle.codegen.serializable;
                ...
                gen.printLine("var registry = Environment.getPublished(SerializableTypesRegistry.class);");
                  for(SerializableType type: registry.getTypes().values()){
                   ...
                   gen.wrapWithBlock("",()->{
                       ...
                       if(type.getJavaQualifiedName() != null){
                           gen.printLine("type.setReadonlyJavaQualifiedName(\"%s\");".formatted(type.getReadonlyJavaQualifiedName()));
                       }
                       if(type.isFinalField()){
                           gen.printLine("type.setFinalField(true);");
                       }
