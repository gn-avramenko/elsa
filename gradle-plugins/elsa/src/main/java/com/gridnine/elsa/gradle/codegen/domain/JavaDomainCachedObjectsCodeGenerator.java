...
public class JavaDomainCachedObjectsCodeGenerator {
    ...
    private static void generateCachedEntity(EntityDescription ed, DomainMetaRegistry totalRegistry, File destDir, Set<File> generatedFiles) throws Exception {
        ...
        cnsb.append("public class _Cached%s extends %s implements CachedObject".formatted(CodeGeneratorUtils.getSimpleName(ed.getId()),
        ...
        gen.wrapWithBlock(cnsb.toString(), () -> {
            ...
            gen.wrapWithBlock("public void setAllowChanges(boolean allowChanges)", () -> {
                gen.printLine("this.allowChanges = allowChanges;");
            });
            for (StandardPropertyDescription pd : allProperties) {
                ...
                gen.wrapWithBlock("public void set%s(%s value)".formatted(BuildTextUtils.capitalize(pd.getId()), className), () -> {
                    gen.wrapWithBlock("if(!allowChanges)", () -> {
                        gen.printLine("throw Xeption.forDeveloper(\"changes are not allowed\");");
                    });
