...
public class JdbcStructureUpdater {
    ...
    @Autowired
    public void setMetadataProvider(DatabaseMetadataProvider metadataProvider){
    ...
    public void init(){
        ...
        final DatabaseStructureAnalysisResult analysisResult = analyze();
    ...
    private void createIndexes(String tableName, Map<String, IndexDescription> indexes) {
    ...
    private DatabaseStructureAnalysisResult analyze() {
        var tablesToCreate = new LinkedHashSet<CreateTableData>();
        ...
        var tablesToUpdate = new LinkedHashSet<UpdateTableData>();
        for (var tableName : exclusion(existingTableNames, tablesToDelete)) {
            ...
            var columnsToCreate = new LinkedHashMap<String, SqlType>();
            ...
            var indexesToCreate = new LinkedHashMap<String, IndexDescription>();
            ...
            tablesToUpdate.add(new UpdateTableData(tableName, columnsToCreate, indexesToCreate, columnsToDelete, indexesToDelete));
        }
        var existingSequencesNames = dialect.geSequencesNames();
        var newSequences = metadataProvider.getSequencesMap().values();
        var newSequencesName = newSequences.stream().map(SequenceDescription::sequenceName).collect(Collectors.toSet());
        ...
        return new DatabaseStructureAnalysisResult(tablesToDelete, tablesToCreate, tablesToUpdate, sequencesToDelete, sequencesToCreate);
    }

    private void removeIgnoreCase(Map<String, ?> existingColumns, String key1) {
        var key2 = existingColumns.keySet().stream().filter(it -> it.equalsIgnoreCase(key1)).findFirst().orElse(null);
        if(key2 != null){
            existingColumns.remove(key2);
        }
    }
    ...
    private CreateTableData toTableData(DatabaseTableDescription databaseTableDescription) {
        var columns = new LinkedHashMap<String, SqlType>();
        var indexes = new LinkedHashMap<String, IndexDescription>();
        ...
        return new CreateTableData(databaseTableDescription.getName(), columns, indexes);
    }
