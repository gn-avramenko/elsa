...
import com.gridnine.elsa.server.core.storage.jdbc.model.JdbcUtils;
...
public class JdbcEnumMapperImpl implements EnumMapper {
            ...
            for(var enumItem: it.getItems().values()){
                ...
                if(row == null){
                    ...
                    while (id == null || found) {
                        ...
                        var id2 = id;
                        found = res.stream().anyMatch(it2 -> id2.equals(it2.get("id")));
                    }
    ...
    public int getId(Enum<?> value) {
        ...
        if(!name2Id.containsKey(className)){
            throw Xeption.forDeveloper("unknown enum %s".formatted(className));
        }
