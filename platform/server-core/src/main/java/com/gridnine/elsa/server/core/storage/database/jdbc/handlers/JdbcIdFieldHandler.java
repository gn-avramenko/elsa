...
public class JdbcIdFieldHandler extends BaseJdbcSingleFieldHandler {
    ...
    public JdbcIdFieldHandler(boolean longId) {
        super(BaseIdentity.Fields.id, false, longId? JdbcFieldType.LONG_ID: JdbcFieldType.INT_ID);
    }
    ...
    public Object getModelValue(ResultSet rs, EnumMapper enumMapper, ClassMapper classMapper, ReflectionFactory factory, JdbcDialect dialect) throws Exception {
        return JdbcUtils.isNull(rs, fieldName)? null : type == JdbcFieldType.LONG_ID? rs.getLong(fieldName): rs.getInt(fieldName);
    }
