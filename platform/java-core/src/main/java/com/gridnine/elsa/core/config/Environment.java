...
public class Environment {
    ...
    public static <T, I extends T> void publish(Class<T> cls, I obj) {
        if (publishedObjects.containsKey(obj.getClass())) {
            throw Xeption.forDeveloper("object of class %s is already published".formatted(cls.getName()));
        }
    ...
    public static <T> T getPublished(Class<T> cls){
        ...
        if(result == null){
            throw Xeption.forDeveloper("object of class %s is not not published".formatted(cls.getName()));
        }
        return (T) result;
    }
